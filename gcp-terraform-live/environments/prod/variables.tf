...
variable "environment" {
  ...
  validation {
    ...
    error_message = "Must be prod for this configuration"
  }
}
...
variable "terraform_service_account" {
  ...
  validation {
    condition     = can(regex("^terraform-prod@[a-z0-9-]{6,30}\\.iam\\.gserviceaccount\\.com$", var.terraform_service_account))
    error_message = "Terraform service account must be the prod Terraform service account for this project."
  }
}
...
variable "web_vm_count" {
  ...
  validation {
    condition     = var.web_vm_count >= 2 && var.web_vm_count <= 10
    error_message = "Prod web VM count must be between 2 and 10."
  }
}
...
variable "web_vm_names" {
  ...
  validation {
    condition     = length(var.web_vm_names) == var.web_vm_count && alltrue([for name in var.web_vm_names : can(regex("^prod-[a-z][a-z0-9-]{1,61}[a-z0-9]$", name))])
    error_message = "Web VM names must match web_vm_count and use prod-prefixed RFC1035-compatible names."
  }
}
...
variable "db_password_secret_id" {
  ...
  sensitive   = true
  validation {
    condition     = can(regex("^projects/[a-z0-9-]{6,30}/secrets/db-password-prod/versions/(latest|[0-9]+)$", var.db_password_secret_id))
    error_message = "Database password secret must reference the prod database password secret version."
  }
}
