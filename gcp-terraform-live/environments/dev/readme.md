...
`backend.hcl` provides the dev-specific GCS state location:
...
```bash
terraform init \
  -backend-config="bucket=${STATE_PROJECT_ID}-terraform-state-dev" \
...
```bash
gsutil ls gs://gcplearning-15042026-terraform-state-dev
```

This environment is configured for project `gcplearning-15042026`. Update `backend.hcl` and `terraform.tfvars` if you move dev to a different project.
